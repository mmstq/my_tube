...
class _VideoPlayerItemState extends State<VideoPlayerItem> {
  ...
  void didUpdateWidget(VideoPlayerItem oldWidget) {
    ...
    // If autoplay status changed and player is initialized
    if (widget.autoplay != oldWidget.autoplay && _isInitialized) {
      if (widget.autoplay) {
        // Always ensure we play when autoplay becomes true
        _controller.play();
        setState(() {
          _isPlaying = true;
        });
      } else if (!widget.autoplay && _isPlaying) {
        // Pause if autoplay becomes false and we're playing
        _controller.pause();
        setState(() {
          _isPlaying = false;
        });
  ...
  Future<void> _initializeVideoController() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        // Auto-play video after initialization if this is the current video
        if (widget.autoplay) {
          _controller.play();
          setState(() {
            _isPlaying = true;
      ...
      final endMessage =
          e.toString().contains('Source error') ? 'Source error' : e.toString();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error playing video: $endMessage'),
  ...
  Widget build(BuildContext context) {
    return Stack(
      ...
      children: [
        ...
        // Play button overlay - only handles tap without consuming vertical swipes
        if (_isInitialized)
          Positioned.fill(
        ...
        // UI Elements (don't interfere with scroll gestures)
